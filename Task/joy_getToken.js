...
[rewrite_local]
# 宠汪汪助力更新Token
^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/addUser\?code= url script-response-body joy_getToken.js
# 宠汪汪助力获取Token
^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/user\/detail\?openId= url script-request-header joy_getToken.js

*/
const $ = new Env('获取来客有礼Token');
...
function getToken() {
  ...
  if (isURL(url, /^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/addUser\?code=/)) {
    ...
    if ($request && $request.method !== 'OPTIONS') {
      ...
      $.msg($.name, '获取Token: 成功🎉', `${jdJoyRunToken}`);
